...
`tests_cdp1802`: unit tests for CDP1802 memory reference instructions (LDI, LDN, STR, STXD) with nMWR pulse timing verification, register operations (INC, DEC, IRX, GLO, GHI, PLO, PHI), logic operations (OR, ORI, XOR, XRI, AND, ANI, SHR, SHRC, SHL, SHLC), arithmetic operations (ADD, ADI, ADC, ADCI, SD, SDI, SDB, SDBI, SM, SMI, SMB, SMBI) with carry/borrow/DF flag verification
