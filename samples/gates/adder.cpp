...
int main()
{
    ...
    Xor xor1{ Xor::Descriptor{ .name = "XOR1" } }; // A ^ B
    Xor xor2{ Xor::Descriptor{ .name = "XOR2" } }; // (A ^ B) ^ Cin -> S
    And and1{ And::Descriptor{ .name = "AND1" } }; // A & B
    And and2{ And::Descriptor{ .name = "AND2" } }; // Cin & (A ^ B)
    Or or1{ Or::Descriptor{ .name = "OR1" } };     // (A & B) | (Cin & (A ^ B)) -> Cout
