#include "nfx/silicon/buffer/cd4xxx/CD4049UB.h"
...
namespace nfx::silicon::buffer::cd4xxx
{
    ...
    CD4049UB::CD4049UB(const Descriptor& descriptor)
        : m_descriptor{ descriptor },
          m_gates{ primitive::Not{ primitive::Not::Descriptor{ .name = "NOT1" } },
                   primitive::Not{ primitive::Not::Descriptor{ .name = "NOT2" } },
                   primitive::Not{ primitive::Not::Descriptor{ .name = "NOT3" } },
                   primitive::Not{ primitive::Not::Descriptor{ .name = "NOT4" } },
                   primitive::Not{ primitive::Not::Descriptor{ .name = "NOT5" } },
                   primitive::Not{ primitive::Not::Descriptor{ .name = "NOT6" } } },
                     ...
                     &m_pinVSS, &m_pinD, &m_pinJ, &m_pinE, &m_pinK, &m_pinF, &m_pinL }
    {
        auto wireGate = [this](const std::size_t idx, signal::Pin& pinIn, signal::Pin& pinOut) {
            pinIn.connect<Level>([this, idx](const Level l) {
                ...
                m_gates[idx].pin(primitive::Not::Terminal::A).drive<Level>(l);
            });

            m_gates[idx].pin(primitive::Not::Terminal::Y).connect<Level>([this, &pinOut](const Level l) {
                if (!m_powered)
                {
                    return;
                }
