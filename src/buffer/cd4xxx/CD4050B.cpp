#include "nfx/silicon/buffer/cd4xxx/CD4050B.h"
...
namespace nfx::silicon::buffer::cd4xxx
{
    ...
    CD4050B::CD4050B(const Descriptor& descriptor)
    ...
    {
        auto wireBuffer = [this](signal::Pin& pinIn, signal::Pin& pinOut) {
            pinIn.connect<Level>([this, &pinOut](const Level l) {
                if (!m_powered)
                {
                    return;
                }
        ...
        wireBuffer(m_pinA, m_pinG);
        wireBuffer(m_pinB, m_pinH);
        wireBuffer(m_pinC, m_pinI);
        wireBuffer(m_pinD, m_pinJ);
        wireBuffer(m_pinE, m_pinK);
        wireBuffer(m_pinF, m_pinL);

        m_pinVCC.connect<float>([this](const Voltage v) { onVCC(v); });
    }

    const char* CD4050B::name() const
    {
        return m_descriptor.name;
    }

    signal::Pin& CD4050B::pin(const char* name)
    {
        ...
        internal::runtime::error::log(
            "CD4050B", internal::runtime::error::Level::Critical, Kind::Contract, "unknown pin name");
        assert(false && "CD4050B: unknown pin name");
        return *m_pinPtrs[0];
    }

    signal::Pin& CD4050B::pin(const Terminal terminal)
    {
        switch (terminal)
        {
            case Terminal::VCC:
        ...
        internal::runtime::error::log(
            "CD4050B", internal::runtime::error::Level::Critical, Kind::Contract, "invalid terminal");
        assert(false && "CD4050B: invalid terminal");
        return *m_pinPtrs[0];
    }

    std::span<signal::Pin* const> CD4050B::pins() const
    {
        return m_pinPtrs;
    }

    void CD4050B::onVCC(const Voltage v)
    {
