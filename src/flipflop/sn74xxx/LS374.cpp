#include "nfx/silicon/flipflop/sn74xxx/LS374.h"
...
namespace nfx::silicon::flipflop::sn74xxx
{
    ...
    LS374::LS374(const Descriptor& descriptor)
        : m_descriptor{ descriptor },
          m_flipflops{ primitive::DFlipFlop{ primitive::DFlipFlop::Descriptor{ .name = "FF0" } },
                       primitive::DFlipFlop{ primitive::DFlipFlop::Descriptor{ .name = "FF1" } },
                       primitive::DFlipFlop{ primitive::DFlipFlop::Descriptor{ .name = "FF2" } },
                       primitive::DFlipFlop{ primitive::DFlipFlop::Descriptor{ .name = "FF3" } },
                       primitive::DFlipFlop{ primitive::DFlipFlop::Descriptor{ .name = "FF4" } },
                       primitive::DFlipFlop{ primitive::DFlipFlop::Descriptor{ .name = "FF5" } },
                       primitive::DFlipFlop{ primitive::DFlipFlop::Descriptor{ .name = "FF6" } },
                       primitive::DFlipFlop{ primitive::DFlipFlop::Descriptor{ .name = "FF7" } } },
                     ...
                     &m_pinQ6, &m_pinQ7, &m_pinD7,  &m_pinD8,  &m_pinQ8, &m_pinVCC }
    {
        ...
        m_pinCLK.connect<Level>([this](Level l) {
            ...
            {
                ff.pin(primitive::DFlipFlop::Terminal::CLK).drive<Level>(l);
            }
        });
        ...
        auto wireFF = [this](std::size_t idx, signal::Pin& pinD, signal::Pin& pinQ) {
            pinD.connect<Level>([this, idx](Level l) {
                if (!m_powered)
                    return;
                m_flipflops[idx].pin(primitive::DFlipFlop::Terminal::D).drive<Level>(l);
            });

            // internal Q -> package output pin
            m_flipflops[idx].pin(primitive::DFlipFlop::Terminal::Q).connect<Level>([this, &pinQ](Level l) {
    ...
    void LS374::onOC(Level /*level*/)
    {
        ...
        {
            const Level ffQ = m_flipflops[i].pin(primitive::DFlipFlop::Terminal::Q).read<Level>();
