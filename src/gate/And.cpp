...
{
    And::And(const Descriptor& descriptor)
    ...
    const char* And::name() const
    {
        return m_descriptor.name;
    }

    signal::Pin& And::pin(const char* name)
    {
        ...
        internal::runtime::error::log(
            ...
            "unknown pin name");
        assert(false && "And: unknown pin name");
        return *m_pinPtrs[0];
    }

    signal::Pin& And::pin(Terminal terminal)
    {
        switch (terminal)
        {
            case Terminal::A:
        ...
        internal::runtime::error::log(
            ...
            internal::runtime::error::Kind::Programming,
            "invalid terminal");
        assert(false && "And: invalid terminal");
        return m_pinA;
    }

    std::span<signal::Pin* const> And::pins() const
    {
        return m_pinPtrs;
    }
