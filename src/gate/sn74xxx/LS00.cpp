...
namespace nfx::silicon::gate::sn74xxx
{
    ...
    LS00::LS00(const Descriptor& descriptor)
                     ...
                     &m_pinY3, &m_pinA3, &m_pinB3, &m_pinY4, &m_pinA4, &m_pinB4, &m_pinVCC }
    {
        auto wireGate = [this](const std::size_t idx, signal::Pin& pinA, signal::Pin& pinB, signal::Pin& pinY) {
            ...
            m_gates[idx].pin(primitive::Nand::Terminal::Y).connect<Level>([this, &pinY](const Level l) {
                if (!m_powered)
                {
                    return;
                }
    ...
    const char* LS00::name() const
    {
        return m_descriptor.name;
    }

    signal::Pin& LS00::pin(const char* name)
    {
        ...
        internal::runtime::error::log(
            "LS00", internal::runtime::error::Level::Critical, Kind::Contract, "unknown pin name");
        assert(false && "LS00: unknown pin name");
        return *m_pinPtrs[0];
    }

    signal::Pin& LS00::pin(const Terminal terminal)
    {
        switch (terminal)
        {
            case Terminal::A1:
        ...
        internal::runtime::error::log(
            "LS00", internal::runtime::error::Level::Critical, Kind::Contract, "invalid terminal");
        assert(false && "LS00: invalid terminal");
        return *m_pinPtrs[0];
    }

    std::span<signal::Pin* const> LS00::pins() const
    {
        return m_pinPtrs;
    }

    void LS00::onVCC(const Voltage v)
    {
