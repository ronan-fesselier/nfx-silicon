...
namespace nfx::silicon::gate::sn74xxx
{
    ...
    LS04::LS04(const Descriptor& descriptor)
        : m_descriptor{ descriptor },
          m_inverters{ primitive::Not{ primitive::Not::Descriptor{ .name = "INV1" } },
                       primitive::Not{ primitive::Not::Descriptor{ .name = "INV2" } },
                       primitive::Not{ primitive::Not::Descriptor{ .name = "INV3" } },
                       primitive::Not{ primitive::Not::Descriptor{ .name = "INV4" } },
                       primitive::Not{ primitive::Not::Descriptor{ .name = "INV5" } },
                       primitive::Not{ primitive::Not::Descriptor{ .name = "INV6" } } },
                     ...
                     &m_pinY4, &m_pinA4, &m_pinY5, &m_pinA5, &m_pinY6, &m_pinA6, &m_pinVCC }
    {
        auto wireInverter = [this](const std::size_t idx, signal::Pin& pinA, signal::Pin& pinY) {
            pinA.connect<Level>([this, idx](const Level l) {
                ...
                m_inverters[idx].pin(primitive::Not::Terminal::A).drive<Level>(l);
            });

            m_inverters[idx].pin(primitive::Not::Terminal::Y).connect<Level>([this, &pinY](const Level l) {
                if (!m_powered)
                {
                    return;
                }
