...
namespace nfx::silicon::gate::sn74xxx
{
    ...
    LS08::LS08(const Descriptor& descriptor)
        : m_descriptor{ descriptor },
          m_gates{ primitive::And{ primitive::And::Descriptor{ .name = "AND1" } },
                   primitive::And{ primitive::And::Descriptor{ .name = "AND2" } },
                   primitive::And{ primitive::And::Descriptor{ .name = "AND3" } },
                   primitive::And{ primitive::And::Descriptor{ .name = "AND4" } } },
                     ...
                     &m_pinY3, &m_pinA3, &m_pinB3, &m_pinY4, &m_pinA4, &m_pinB4, &m_pinVCC }
    {
        auto wireGate = [this](const std::size_t idx, signal::Pin& pinA, signal::Pin& pinB, signal::Pin& pinY) {
            pinA.connect<Level>([this, idx](const Level l) {
                ...
                m_gates[idx].pin(primitive::And::Terminal::A).drive<Level>(l);
            });
            ...
            pinB.connect<Level>([this, idx](const Level l) {
                ...
                m_gates[idx].pin(primitive::And::Terminal::B).drive<Level>(l);
            });

            m_gates[idx].pin(primitive::And::Terminal::Y).connect<Level>([this, &pinY](const Level l) {
                if (!m_powered)
                {
                    return;
                }
    ...
    const char* LS08::name() const
    {
        return m_descriptor.name;
    }

    signal::Pin& LS08::pin(const char* name)
    {
        ...
        internal::runtime::error::log(
            "LS08", internal::runtime::error::Level::Critical, Kind::Contract, "unknown pin name");
        assert(false && "LS08: unknown pin name");
        return *m_pinPtrs[0];
    }

    signal::Pin& LS08::pin(const Terminal terminal)
    {
        switch (terminal)
        {
            case Terminal::A1:
        ...
        internal::runtime::error::log(
            "LS08", internal::runtime::error::Level::Critical, Kind::Contract, "invalid terminal");
        assert(false && "LS08: invalid terminal");
        return *m_pinPtrs[0];
    }

    std::span<signal::Pin* const> LS08::pins() const
    {
        return m_pinPtrs;
    }

    void LS08::onVCC(const Voltage v)
    {
