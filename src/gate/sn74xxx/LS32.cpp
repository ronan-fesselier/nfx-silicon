...
namespace nfx::silicon::gate::sn74xxx
{
    ...
    LS32::LS32(const Descriptor& descriptor)
        : m_descriptor{ descriptor },
          m_gates{ primitive::Or{ primitive::Or::Descriptor{ .name = "OR1" } },
                   primitive::Or{ primitive::Or::Descriptor{ .name = "OR2" } },
                   primitive::Or{ primitive::Or::Descriptor{ .name = "OR3" } },
                   primitive::Or{ primitive::Or::Descriptor{ .name = "OR4" } } },
                     ...
                     &m_pinY3, &m_pinA3, &m_pinB3, &m_pinY4, &m_pinA4, &m_pinB4, &m_pinVCC }
    {
        auto wireGate = [this](const std::size_t idx, signal::Pin& pinA, signal::Pin& pinB, signal::Pin& pinY) {
            pinA.connect<Level>([this, idx](const Level l) {
                ...
                m_gates[idx].pin(primitive::Or::Terminal::A).drive<Level>(l);
            });
            ...
            pinB.connect<Level>([this, idx](const Level l) {
                ...
                m_gates[idx].pin(primitive::Or::Terminal::B).drive<Level>(l);
            });

            m_gates[idx].pin(primitive::Or::Terminal::Y).connect<Level>([this, &pinY](const Level l) {
                if (!m_powered)
                {
                    return;
                }
    ...
    const char* LS32::name() const
    {
        return m_descriptor.name;
    }

    signal::Pin& LS32::pin(const char* name)
    {
        ...
        internal::runtime::error::log(
            "LS32", internal::runtime::error::Level::Critical, Kind::Contract, "unknown pin name");
        assert(false && "LS32: unknown pin name");
        return *m_pinPtrs[0];
    }

    signal::Pin& LS32::pin(const Terminal terminal)
    {
        switch (terminal)
        {
            case Terminal::A1:
        ...
        internal::runtime::error::log(
            "LS32", internal::runtime::error::Level::Critical, Kind::Contract, "invalid terminal");
        assert(false && "LS32: invalid terminal");
        return *m_pinPtrs[0];
    }

    std::span<signal::Pin* const> LS32::pins() const
    {
        return m_pinPtrs;
    }

    void LS32::onVCC(const Voltage v)
    {
