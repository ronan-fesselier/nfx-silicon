...
namespace nfx::silicon::gate::sn74
{
    ...
    LS373::LS373(const Descriptor& descriptor)
        : m_descriptor{ descriptor },
          m_latches{ DLatch{ DLatch::Descriptor{ .name = "L0" } }, DLatch{ DLatch::Descriptor{ .name = "L1" } },
                     DLatch{ DLatch::Descriptor{ .name = "L2" } }, DLatch{ DLatch::Descriptor{ .name = "L3" } },
                     DLatch{ DLatch::Descriptor{ .name = "L4" } }, DLatch{ DLatch::Descriptor{ .name = "L5" } },
                     DLatch{ DLatch::Descriptor{ .name = "L6" } }, DLatch{ DLatch::Descriptor{ .name = "L7" } } },
                     ...
                     &m_pinQ6, &m_pinQ7, &m_pinD7,  &m_pinD8, &m_pinQ8, &m_pinVCC }
    {
        ...
        auto wireLatch = [this](const std::size_t idx, signal::Pin& pinD, signal::Pin& pinQ) {
            pinD.connect<Level>([this, idx](const Level l) {
                ...
                m_latches[idx].pin(DLatch::Terminal::D).drive<Level>(l);
            });

            m_latches[idx].pin(DLatch::Terminal::Q).connect<Level>([this, &pinQ](const Level l) {
        ...
        m_pinC.connect<Level>([this](const Level l) {
            ...
            {
                latch.pin(DLatch::Terminal::EN).drive<Level>(l);
            }
        });
    ...
    void LS373::onVCC(const Voltage v)
    {
        ...
        if (!wasPowered && m_powered)
        {
            ...
            {
                latch.reset();
                latch.pin(DLatch::Terminal::EN).drive<Level>(m_pinC.read<Level>());
            }
    ...
    void LS373::onOC(const Level /*level*/)
    {
        ...
        {
            const Level latchQ = m_latches[i].pin(DLatch::Terminal::Q).read<Level>();
