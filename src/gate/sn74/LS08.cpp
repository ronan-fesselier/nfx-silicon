...
namespace nfx::silicon::gate::sn74
{
    ...
    LS08::LS08(const Descriptor& descriptor)
        : m_descriptor{ descriptor },
          m_gates{ And{ And::Descriptor{ .name = "AND1" } },
                   And{ And::Descriptor{ .name = "AND2" } },
                   And{ And::Descriptor{ .name = "AND3" } },
                   And{ And::Descriptor{ .name = "AND4" } } },
                     ...
                     &m_pinY3, &m_pinA3, &m_pinB3, &m_pinY4, &m_pinA4, &m_pinB4, &m_pinVCC }
    {
        auto wireGate = [this](const std::size_t idx, signal::Pin& pinA, signal::Pin& pinB, signal::Pin& pinY) {
            pinA.connect<Level>([this, idx](const Level l) {
                ...
                m_gates[idx].pin(And::Terminal::A).drive<Level>(l);
            });
            ...
            pinB.connect<Level>([this, idx](const Level l) {
                ...
                m_gates[idx].pin(And::Terminal::B).drive<Level>(l);
            });

            m_gates[idx].pin(And::Terminal::Y).connect<Level>([this, &pinY](const Level l) {
                if (!m_powered)
                {
                    return;
                }
