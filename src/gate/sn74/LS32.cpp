...
namespace nfx::silicon::gate::sn74
{
    ...
    LS32::LS32(const Descriptor& descriptor)
        : m_descriptor{ descriptor },
          m_gates{ Or{ Or::Descriptor{ .name = "OR1" } },
                   Or{ Or::Descriptor{ .name = "OR2" } },
                   Or{ Or::Descriptor{ .name = "OR3" } },
                   Or{ Or::Descriptor{ .name = "OR4" } } },
                     ...
                     &m_pinY3, &m_pinA3, &m_pinB3, &m_pinY4, &m_pinA4, &m_pinB4, &m_pinVCC }
    {
        auto wireGate = [this](const std::size_t idx, signal::Pin& pinA, signal::Pin& pinB, signal::Pin& pinY) {
            pinA.connect<Level>([this, idx](const Level l) {
                ...
                m_gates[idx].pin(Or::Terminal::A).drive<Level>(l);
            });
            ...
            pinB.connect<Level>([this, idx](const Level l) {
                ...
                m_gates[idx].pin(Or::Terminal::B).drive<Level>(l);
            });

            m_gates[idx].pin(Or::Terminal::Y).connect<Level>([this, &pinY](const Level l) {
                if (!m_powered)
                {
                    return;
                }
