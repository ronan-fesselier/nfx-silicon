#include "nfx/silicon/primitive/Nor.h"
...
{
    Nor::Nor(const Descriptor& descriptor)
    ...
    const char* Nor::name() const
    {
        return m_descriptor.name;
    }

    signal::Pin& Nor::pin(const char* name)
    {
        ...
        internal::runtime::error::log(
            ...
            "unknown pin name");
        assert(false && "Nor: unknown pin name");
        return *m_pinPtrs[0];
    }

    signal::Pin& Nor::pin(Terminal terminal)
    {
        switch (terminal)
        {
            case Terminal::A:
        ...
        internal::runtime::error::log(
            ...
            internal::runtime::error::Kind::Programming,
            "invalid terminal");
        assert(false && "Nor: invalid terminal");
        return m_pinA;
    }

    std::span<signal::Pin* const> Nor::pins() const
    {
        return m_pinPtrs;
    }
    ...
    void Nor::compute()
    {
        ...
        m_pinY.drive<Level>((a == Level::Low && b == Level::Low) ? Level::High : Level::Low);
    }
} // namespace nfx::silicon::primitive
