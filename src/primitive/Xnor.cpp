#include "nfx/silicon/primitive/Xnor.h"
...
{
    Xnor::Xnor(const Descriptor& descriptor)
    ...
    const char* Xnor::name() const
    {
        return m_descriptor.name;
    }

    signal::Pin& Xnor::pin(const char* name)
    {
        ...
        internal::runtime::error::log(
            ...
            "unknown pin name");
        assert(false && "Xnor: unknown pin name");
        return *m_pinPtrs[0];
    }

    signal::Pin& Xnor::pin(Terminal terminal)
    {
        switch (terminal)
        {
            case Terminal::A:
        ...
        internal::runtime::error::log(
            ...
            internal::runtime::error::Kind::Programming,
            "invalid terminal");
        assert(false && "Xnor: invalid terminal");
        return m_pinA;
    }

    std::span<signal::Pin* const> Xnor::pins() const
    {
        return m_pinPtrs;
    }
